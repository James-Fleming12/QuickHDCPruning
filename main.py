...
# Test if saved model works on training data
def test_model_consistency(dim: int):
    batch_size = 64
    transform = transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize((0.1307,), (0.3081,))
    ...
    # Load the model
    hdc = HDCImageClassifier(input_channels=1, hd_dim=dim)
    hdc.load_state_dict(torch.load(new_hdc_path))

    print(f"\nDEBUG AFTER LOADING:")
    print(f"  hdc.hdc.prototype_accum shape: {hdc.hdc.prototype_accum.shape}")
    print(f"  hdc.hdc.class_counts: {hdc.hdc.class_counts}")
    print(f"  hdc.hdc.prototypes shape: {hdc.hdc.prototypes.shape}")
    
    train_dataset: Dataset = torchvision.datasets.MNIST(root=data_dir, train=True, download=True, transform=transform)
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=2)
    
    # Test on a small batch from training data
    images, labels = next(iter(train_loader))
    images, labels = images.to(hdc.device), labels.to(hdc.device)
    
    print("Testing on TRAINING data:")
    with torch.no_grad():
        preds, sims = hdc.predict(images)
        acc = (preds == labels).float().mean().item()
        print(f"Accuracy on training batch: {acc*100:.2f}%")
